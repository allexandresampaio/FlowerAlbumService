 ...
 * @author alunoifba
 */
@WebServlet(name = "CalculatorWSServlet", urlPatterns = {"/CalculatorWSServlet"})
public class CalculatorWSServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int bodyText = Integer.parseInt(TextArea1);
            int bodyText2 = Integer.parseInt(TextArea2);

            //Process result
            int doc = -1;
            if (request.getParameter("button").equals("+")) {
                doc = add(bodyText, bodyText2);
            } else if (request.getParameter("button").equals("-")) {
                doc = sub(bodyText, bodyText2);
            } else if (request.getParameter("button").equals("*")) {
                doc = mult(bodyText, bodyText2);
            } else if (request.getParameter("button").equals("/")) {
                doc = div(bodyText, bodyText2);
            }
...
//            List allwrongwords = doc.getMisspelledWord();
            ...
            out.println("<title>Calculator Report</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h2>Retorno: " + doc + "</h2>");
